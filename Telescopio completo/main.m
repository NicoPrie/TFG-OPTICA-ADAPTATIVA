...
tfin=60*3; %Segundo final de la simulación.
...
for i=1:length(tiempo)
    ...
    [~, AltAzSol(i,:)]=AngFCordT([40.437271,-3.714715],[0,0,0],[0,0,0],[2026, 93, 14+tiempo(i)/(60*60)]);
end
...
slew_rate = 3;  % degrees per second
start_pos = [0, 0];
delta = AltAzSol(1,:) - start_pos;
T_ramp = (max(abs(delta))-20) / slew_rate;
ramp_idx = find(tiempo <= T_ramp);

AltAzSol_ramped = AltAzSol;
AltAzSol_ramped(ramp_idx, 1) = linspace(start_pos(1), AltAzSol(1,1), length(ramp_idx));
AltAzSol_ramped(ramp_idx, 2) = linspace(start_pos(2), AltAzSol(1,2), length(ramp_idx));

[~, AltAzESP]=SimPID(tiempo, AltAzSol_ramped, "viento", false);
disp("Posiciones del telescopio calculadas.")

% Comparación de errores
error_base = AltAzSol - [AltAzESP_base(:,1), AltAzESP_base(:,3)];
fprintf('RMS sin adelanto:  Alt=%.6f°  Az=%.6f°\n', rms(error_base(:,1)), rms(error_base(:,2)))
...
% mask = AltAzSol - [AltAzESP(:,1), AltAzESP(:,3)] > 1;
% AltAzSol_SisSegmg(mask) = 0;
...
for i=2:length(rESP0)
    ...
    for j=1:length(tiempo)
        ...
        AngEsp(j,:)=[Az,El];
        if abs(AltAzSol(j,:) - [AltAzESP(j,1), AltAzESP(j,3)]) > 0.1
           AngEsp(j,:)=[0,0];
        end
    end
...
%%
dibESPEJO(tiempo, AltAzESP, AltAzSol, AltAzSEG, AltAzSEGobj, Pint, vel=5, fps=20, trayectoria=1, rESP0=rESP0, record=0, view=1) %colorerror=1, error_max=0.0075)
%%
