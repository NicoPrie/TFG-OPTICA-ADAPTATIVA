...
[~, AltAzESP_base]=SimPID(tiempo, AltAzSol, "viento", true);
[~, AltAzESP_pred]=SimPID(tiempo, AltAzSol, "viento", true, "adelanto", 150);
AltAzESP=AltAzESP_pred;
disp("Posiciones del telescopio calculadas.")

% Comparación de errores
error_base = AltAzSol - [AltAzESP_base(:,1), AltAzESP_base(:,3)];
error_pred = AltAzSol - [AltAzESP_pred(:,1), AltAzESP_pred(:,3)];

fprintf('RMS sin adelanto:  Alt=%.6f°  Az=%.6f°\n', rms(error_base(:,1)), rms(error_base(:,2)))
fprintf('RMS con adelanto:  Alt=%.6f°  Az=%.6f°\n', rms(error_pred(:,1)), rms(error_pred(:,2)))
